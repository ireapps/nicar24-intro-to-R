...
-   **code chunk**: created by typing `ctrl+alt+i` (on a PC), type your code inside code chunks.
...
Using the Files pane in the lower-right quadrant, click once on the `scripts` folder and then on the filename `R-Basics.Rmd`. It should open in the upper-left quadrant.

Some of the code is already written out. You can run it by clicking the green "play" button in the upper-right corner of a code chunk. You can also run individual lines of code in a code chunk by putting your cursor on that line and typing `ctrl+enter`.
